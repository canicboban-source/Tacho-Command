...
test("product version manifest exposes factual independent surfaces", () => {
  assert.equal(TACHOCOMMAND_VERSIONS.product, "1.0.0-beta.4");
  assert.equal(TACHOCOMMAND_VERSIONS.site, "1.0.0-beta.4");
  assert.equal(TACHOCOMMAND_VERSIONS.app, "1.0.0-beta.4");
...
test("visible product surfaces use the shared version manifest", () => {
  ...
  assert.match(line, /Site 1\.0\.0-beta\.4/);
  assert.match(line, /App 1\.0\.0-beta\.4/);
