...
const copy = {
  sr: {
    ...
    valueCards: [
      ...
      ["03", "Čitanje bez nagađanja", "Prikazujemo očitane aktivnosti i vreme. Ne tumačimo pravila pojedinačnih linija."],
    ...
    cockpitItems: [
      ["AMBER", "Preventivno upozorenje", "Pregled vremena i aktivnosti očitanih sa tahografa, bez tumačenja pravila pojedinačnih linija."],
    ...
    trust: [
      ...
      ["Čitanje podataka", "Prikaz aktivnosti i trajanja bez procene pravila pojedinačne linije."],
    ...
    plannedItems: ["iPhone / Safari Web Bluetooth path", "Drugi Smart Tacho 2 modeli bez field testa", "Potpuna kriptografska signature validacija u UI-ju", "Dodatni modeli tahografa bez terenske potvrde"],
  ...
  en: {
    ...
    valueCards: [["01","See what matters now","Activity, driving, break and warnings in one view."],["02","56 days as a timeline","Every day becomes a clear 24-hour strip of DRIVE, WORK, AVAILABILITY and REST."],["03","Read without guessing","See recorded activities and times without route-specific rule interpretations."],["04","Proof before marketing","Compatibility is claimed only after real-device validation."]],
    ...
    cockpitKicker: "SAFETY & COMPLIANCE", cockpitTitle: "A warning should help before it becomes a problem.", cockpitText: "A safety warning and a legal verdict are not the same thing. TachoCommand separates them and uses the rule profile that matches the operation.", cockpitItems: [["AMBER","Preventive warning","View the times and activities read from the tachograph without interpreting route-specific rules."],["LIMIT","Threshold reached","The driver sees the relevant limit without an early infringement label."],["REVIEW","Possible infringement","Historical events get date, time, duration and rule basis only when the legal regime is known."]],
    privacyKicker: "PRIVACY & TRUST", privacyTitle: "Your driver card is not marketing data.", privacyText: "Raw driver-card content is not committed to the public repository. Field logs are sanitised. The UI does not need to expose identity data to deliver a useful 56-day view.", trust: [["Field-first","A feature is proven only after it works on a real tachograph."],["No blind retries","STOP/FAIL states do not hammer the DTCO with repeated requests."],["Data reading","Recorded activities and durations without route-specific legal interpretation."],["Local clarity","Raw bytes become validated normalized data before UI rendering."]],
    compatibilityKicker: "COMPATIBILITY", compatibilityTitle: "We only claim what we have actually proven.", tested: "FIELD TESTED", planned: "PLANNED / NOT YET CLAIMED", testedItems: ["Continental VDO DTCO 4.1a","Smart Tacho 2 BLE Download path","Android + Chrome + HTTPS","Gen2 v2 Driver Card Slot 1 download","56-day activity parsing"], plannedItems: ["iPhone / Safari Web Bluetooth path","Other Smart Tacho 2 models without field tests","Full cryptographic signature validation in UI","Additional tachograph models without field verification"],
    ...
    faqTitle: "Questions beginners actually ask.", faqs: [["Does TachoCommand modify the tachograph?","No. The proven card-download path reads data through the supported Smart Tacho 2 communication path. The tachograph and card remain authoritative."],["Do I need to be technical?","No. Pairing, PIN confirmation, device selection and card reading are guided step by step."],["Do you immediately call something an infringement?","No. Alerts and candidate events are only classified inside a known legal profile."],["Do my raw card files go to GitHub?","No. The personal .ddd field fixture is not committed; tests use synthetic data."]],
  ...
  de: {
    ...
    valueCards: [["01","Das Wichtige sofort sehen","Tätigkeit, Lenkzeit, Pause und Warnungen in einer Ansicht."],["02","56 Tage als Timeline","Jeder Tag wird als klare 24-Stunden-Leiste mit DRIVE, WORK, AVAILABILITY und REST dargestellt."],["03","Auslesen statt raten","Aufgezeichnete Tätigkeiten und Zeiten anzeigen, ohne linienbezogene Regelbewertung."],["04","Nachweis vor Marketing","Kompatibilität gilt erst nach realem Gerätetest als bestätigt."]],
      ...
      <section className="tcx-hero" id="top">
        ...
        <div className="tcx-device-stage" aria-label="TachoCommand cockpit product view">
          ...
          <div className="tcx-phone">
            ...
            <div className="tcx-phone-brand"><span className="tcx-mini-logo">TC</span><strong>TachoCommand</strong><small>DRIVER CARD · 56 DAYS</small></div>
