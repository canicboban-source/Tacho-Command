...
export function createAppV2LiveSession(input = {}) {
  ...
  const continuousDrivingSec = finiteNonNegative(telemetry.continuousDrivingSeconds);
  const dailyDrivingSec = finiteNonNegative(telemetry.dailyDrivingSeconds);
  const weeklyDrivingSec = finiteNonNegative(telemetry.weeklyDrivingSeconds);
  ...
  const productLive = createFieldProvenLiveSnapshot({
    ...
    continuousDrivingSec: continuousDrivingSec ?? undefined,
    dailyDrivingSec: dailyDrivingSec ?? undefined,
    weeklyDrivingSec: weeklyDrivingSec ?? undefined,
    telemetryAcceptedCount: boundedInteger(input.telemetryAcceptedCount, 0, 1000000) ?? undefined,
