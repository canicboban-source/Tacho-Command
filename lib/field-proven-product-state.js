...
function normalizeHistoryDay(input) {
  ...
  for (const segment of rawSegments) {
    ...
    const startMinute = explicitStart ?? cursorMinute;
    const endMinute = explicitEnd ?? Math.min(1440, startMinute + roundedMinutes);
    if (endMinute < startMinute || startMinute > 1440 || endMinute > 1440) continue;
    const positionedMinutes = endMinute - startMinute;
    if (explicitStart !== null && explicitEnd !== null && Math.abs(positionedMinutes - roundedMinutes) > 1) continue;

    const minutes = explicitStart !== null && explicitEnd !== null ? positionedMinutes : roundedMinutes;
...
export function createFieldProvenProductState(input = {}) {
  ...
  return Object.freeze({
    ...
    currentActivity: connected ? normalizeActivity(live.activity) : "UNKNOWN",
    continuousDrivingMinutes: connected ? continuousDrivingMinutes : null,
    continuousProgressPercent: connected ? continuous.progressPercent : null,
    continuousRemainingLabel: connected ? continuous.remainingLabel : null,
    continuousThresholdLabel: connected ? continuous.thresholdLabel : null,
    continuousBand: connected ? continuous.band : "neutral",
    todayDrivingMinutes: connected ? nullableMinutesFromSeconds(live.dailyDrivingSec) : null,
    weekDrivingMinutes: connected ? nullableMinutesFromSeconds(live.weeklyDrivingSec) : null,
