...
function normalizeHistoryDay(input) {
  ...
  const dateIso = typeof input.dateIso === "string" && /^\d{4}-\d{2}-\d{2}$/.test(input.dateIso)
    ? input.dateIso
    : null;
...
export function createFieldProvenProductState(input = {}) {
  ...
  const historyDays = (Array.isArray(card.historyDays) ? card.historyDays : [])
    ...
    .filter(Boolean)
    .sort((left, right) => (right.dateIso ?? right.dateLabel).localeCompare(left.dateIso ?? left.dateLabel));
