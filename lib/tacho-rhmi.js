...
export function parseDriverWorkingState(responseBytes = []) {
  ...
  return Object.freeze({ valid: stateCode <= 3, activity, activityCode: stateCode });
}

export function inspectVehicleSpeedDid(responseBytes = []) {
  const bytes = Array.from(responseBytes, (value) => Number(value) & 0xff);
  if (
    bytes.length < 6
    || bytes[0] !== 1
    ...
    return Object.freeze({ valid: false, payload: Object.freeze([]) });
  }

  return Object.freeze({
    valid: true,
    payload: Object.freeze(bytes.slice(5)),
  });
}

export function classifyStationaryVehicleSpeed(responseBytes = []) {
  const inspected = inspectVehicleSpeedDid(responseBytes);
  const stationary = inspected.valid
    && inspected.payload.length === 2
    && inspected.payload[0] === 0x00
    && inspected.payload[1] === 0x00;

  return Object.freeze({
    valid: inspected.valid && inspected.payload.length === 2,
    stationary,
